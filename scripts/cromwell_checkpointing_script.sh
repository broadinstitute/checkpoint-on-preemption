...
# This is a modification of Ted Brooking's monitoring script,
# adding periodic syncronization functionality for intermediate date
# (e.g. ML training, long-running basecalling, etc.).
#
...
#
# options:
#   "monitoring_script": "gs://bucket/path/to/cromwell_checkpointing_script.sh"
#
# Upon task completion "monitoring.log" will be added to the appropriate
# cloud storage folder.  Additionally, intermediate data will be
# periodically synced to a designated cloud storage location for
# later resumption from the point of interruption.
...
while true; do
    ...
    ((TIME_ELAPSED += 10))
    if [[ $(($TIME_ELAPSED % $SYNC_INTERVAL)) == 0 ]]; then
        echo 'sync time!'
    fi
done
